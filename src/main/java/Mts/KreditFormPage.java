...
public class KreditFormPage {
    private SelenideElement fioInput = $x("//textarea[@name='clientFio']"); // Поле для ввода фамилии имени и отчества клиента
    ...
    private SelenideElement confirmationElement = $x("//div[@class='Wrapper-sc-1vydk7-0 gnJtJP HelperText-sc-jsokzo-0 hByJHf']"); // Элемент предупреждающий о несоответствии каких-то критериев оформления кредита

    @Step("Ввести в поле ФИО значение {fio}")
    public void enterClientFio(String fio) {
 fioInput.scrollTo().hover().setValue(fio);
    }
